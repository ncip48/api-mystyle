...
app.post("/login", (req, res) => {
  ...
  conn.query(sql, (err, results) => {
    ...
    results.length === 1
      ...
      : res.json({ result: 0, error: "username/password salah" });
  });
});
      ...
      conn.query(q3, (err, post) => {
        ...
        conn.query(q4, (err, user) => {
          ...
          conn.query(q5, (err, posting) => {
            let q6 =
              "SELECT a.* FROM tb_produk a JOIN tb_user b ON a.id_penjual=b.id_konsumen WHERE b.username='" +
              username +
              "' AND b.type='1'";
            conn.query(q6, (err, product) => {
              const type = JSON.parse(JSON.stringify(user))[0].type;
              type === 1 ? (product = product) : (product = null);
              res.json({
                result: "1",
                data: {
                  ...
                  jumlah_followers: followers[0].jumlah_followers,
                  jumlah_following: following[0].jumlah_following,
                  jumlah_post: post[0].jumlah_post,
                  user: user[0],
                  post: posting,
                  product: product,
                },
